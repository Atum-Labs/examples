...
- Node.js 20+. Each app runs standalone (`cd x402-accept-payments`); the repo root has orchestration scripts to run every app together. On Node 20, MPP installs may print `EBADENGINE` for transitive deps that declare `>=22` — install and stub runs still succeed; Node 22+ silences the warning.
- **`@atumlabs/*` packages are restricted npm (early access).** `npm login` alone is not enough — the account must be **invited to the `@atumlabs` org with read access**. Packages: `@atumlabs/x402-atum-escrow`, `@atumlabs/mppx-atum-escrow`, `@atumlabs/payment-gateway-client`. Verify with `npm view @atumlabs/x402-atum-escrow version` (prints a version; `404` means no access yet). Restricted installs often surface as `404`, not `403`. This affects both `make` apps, `mpp-accept-payments`, and running any test (tests drive the sibling client). Confirm access before `npm run install:all`.
- Run: merchants use `npm run dev`; payers use `npm run pay`. **Stub mode is the default** — the full `402 → pay → 200` flow runs locally with no gateway, facilitator, or funds. Stub `npm run pay` still needs a **valid** `PRIVATE_KEY` (any throwaway key; no funds).
