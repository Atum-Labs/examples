...
- Verify from the repo root: `npm run typecheck` and `npm test` (smoke, no funds) run all four apps; funded e2e is the [root quickstart](./README.md#root-quickstart) — `export PRIVATE_KEY` + `export DEST_ADDRESS` then `npm run test:e2e` (sets `RUN_REAL_E2E=1`). Both directions of Base ↔ Tempo by default; `SKIP_REVERSE=1` for the forward leg only. One app: `RUN_REAL_E2E=1 PRIVATE_KEY=… DEST_ADDRESS=… npm test` in an `accept` app.
...
- **Real settlement is opt-in and moves real testnet funds.** Root: `npm run test:e2e` (sets `RUN_REAL_E2E=1`). One app: `RUN_REAL_E2E=1 PRIVATE_KEY=… DEST_ADDRESS=… npm test`. By-hand: `USE_STUB_FACILITATOR=false` (x402) / `USE_STUB_SUBMITTER=false` (MPP). Startup guards refuse the placeholder MPP secret and an invalid `DEST_ADDRESS` in real mode — keep those guards intact.
