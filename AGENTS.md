...
Proprietary Atum reference examples for accepting and making payments over **x402** and **MPP**, on the **Base Sepolia USDC ↔ Tempo (Moderato) pathUSD** corridor. Not open source — see [`LICENSE`](./LICENSE).

## Layout

Each app is a standalone npm package (no root package.json, no workspaces). Apps pair by protocol: an `accept` merchant + a `make` payer.
...
- Node.js 20+. Work inside a single app directory (`cd x402-accept-payments`), not the repo root.
...
- Verify: `npm run typecheck` (all apps) and `npm test` (all except `x402-make-payments`, whose e2e lives in `x402-accept-payments`). Tests are Node's built-in runner via `tsx`.
...
- **x402 v1 has no async tail.** On slow corridors (notably Base ↔ Tempo), settlement can outrun the facilitator's ~30s synchronous window; `/settle` then returns `"async tail is not supported in v1"`. Treat that as **pending, not failed** — verify on-chain before retrying. Prefer **MPP** for this corridor (its merchant polls the gateway to a terminal state).
